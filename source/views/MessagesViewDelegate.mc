...
class MessagesViewDelegate extends WatchUi.BehaviorDelegate {
  ...
  function onSwipe(swipeEvent as WatchUi.SwipeEvent) as Lang.Boolean {
    ...
    _logger.debug(
        "MessagesViewDelegate",
        "Received swipe, direction: " + direction.toString());
  ...
  private function goPreviousView() {
    _logger.debug(
        "MessagesViewDelegate", "Switching to (previous) Analog view");

    WatchUi.switchToView(
        new AnalogView(), new AnalogViewDelegate(), WatchUi.SLIDE_LEFT);
    return true;
  }
}